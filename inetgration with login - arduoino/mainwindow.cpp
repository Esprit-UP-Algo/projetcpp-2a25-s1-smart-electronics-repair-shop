...
void MainWindow::readArduinoData()
{
    ...
    qDebug() << "📨 Arduino sent:" << receivedText;
...
// ============================================
// FIXED: onSerialDataReceived() - READS BUTTON PRESSES
// ============================================
/*
...
void MainWindow::on_pushButton_7_clicked()
{
    ...
    connect(&popup, &Contenir::montantTotalCalcule,
            this, &MainWindow::onMontantTotalCalcule);

    // Execute the dialog
    popup.exec();
}
void MainWindow::onMontantTotalCalcule(double montant)
{
    // Update the montant total label in the UI
    ui->lineEdit_49->setReadOnly(true);
    ui->lineEdit_49->setText(QString::number(montant, 'f', 3) + " DT");

    // Also update the line edit if you have one for montant total
    if (ui->lineEdit_49) {
        ui->lineEdit_49->setText(QString::number(montant, 'f', 3));
    }

    qDebug() << "Montant total automatiquement mis à jour:" << montant << "DT";
}
...
void MainWindow::startFinanceAnimation(double valeurPerdue, const QString& marque)
{
    currentValue = valeurPerdue;
    ...
    financeTimer = new QTimer(this);
    connect(financeTimer, &QTimer::timeout, this, &MainWindow::animateFinance);
    financeTimer->start(15);

    animateFinance(); // first frame
}

void MainWindow::animateFinance()
{
    double k = financeAnim / 100.0;
    ...
    double animatedValue = currentValue * k;

    ui->label_12->setText(
        QString("Valeur perdue (%1) : %2 DT")
            .arg(currentMarque)
            .arg(QString::number(animatedValue, 'f', 2))
        );
}
...
void MainWindow::on_btnPage2stock_clicked(){
    ...
    ui->label_16->setText(QString::number(quantiteRupture));

    QMap<QString, int> marqueCounts;

    QSqlQuery query("SELECT MARQUE, SUM(QUANTITE) FROM PRODUITS GROUP BY MARQUE");
    while (query.next()) {
        marqueCounts[query.value(0).toString()] = query.value(1).toInt();
    }
    ...
    for (int i = 0; i < list.size(); ++i) {
        ...
        p.drawText(QRect(x, height - margin + 2, barWidth, 20),
                   Qt::AlignCenter, marqueName);

        x += barWidth + spacing;
    }

    p.end();
    ui->label_6->setPixmap(pix.scaled(ui->label_6->size(),
                                      Qt::KeepAspectRatio,
...
void MainWindow::calculerStatsParMarque()
{
    ...
    for (int i = 0; i < ui->tableWidgetstock->rowCount(); ++i)
    {
        ...
        if (itemExp)
        {
            QDate exp = QDateTime::fromString(itemExp->text().trimmed(), Qt::ISODate).date();
            if (exp.isValid() && exp < today) {
                quantiteExpiree += qte;
    ...
    ui->label_13->setText(QString::number(produitsRupture));
    ...
    ui->label_14->setText(QString("Les produits %1 en rupture").arg(selectedMarque));

    // ⭐ ANIMATED FINANCIAL VALUE ⭐
    startFinanceAnimation(valeurPerdue, selectedMarque);
}
