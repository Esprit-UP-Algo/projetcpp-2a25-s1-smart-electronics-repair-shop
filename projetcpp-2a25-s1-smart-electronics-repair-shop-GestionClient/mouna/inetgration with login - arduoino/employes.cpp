...
bool Employee::ajouter()
{
    ...
    query.prepare("INSERT INTO EMPLOYES (ID, NOM, PRENOM, ABSENCE, NUMTEL, SEXE, ADRESSE, POST, DATE_NAISSANCE, SALAIRE, PWD) "
                  "VALUES (:id, :nom, :prenom, :absence, :numtel, :sexe, :adresse, :poste, :date_naissance, :salaire, :pwd)");
...
{
    QSqlQuery query;
    // Change POSTE to POST
    query.prepare("UPDATE EMPLOYES SET NOM=:nom, PRENOM=:prenom, ABSENCE=:absence, "
                  "NUMTEL=:numtel, SEXE=:sexe, ADRESSE=:adresse, POST=:poste, "
                  "DATE_NAISSANCE=:date_naissance, SALAIRE=:salaire WHERE ID=:id");
    ...
    query.bindValue(":sexe", sexe);
    query.bindValue(":adresse", adresse);
    query.bindValue(":poste", poste);  // Changed from POSTE to POST
    query.bindValue(":date_naissance", date_naissance);
    query.bindValue(":salaire", salaire);
