...
bool Contenir::validerVente()
{
    ...
    QMessageBox::StandardButton confirm = QMessageBox::question(
        this, "Staging Vente",
        QString("Êtes-vous sûr de vouloir stage cette vente #%1 avec %2 produit(s)?\n\n"
                "La vente sera sauvegardée en mémoire temporairement.\n"
                "Vous devrez la commiter plus tard vers la base de données.")
            .arg(currentIdVente).arg(items.count()),
        QMessageBox::Yes | QMessageBox::No
        );
    ...
    qDebug() << "Vente #" << currentIdVente << "staged with"
             << stagedVente.items.count() << "items";

    QMessageBox::information(this, "Vente Staged",
                             QString("Vente #%1 a été staged en mémoire.\n"
                                     "%2 produit(s) seront enregistrés plus tard.")
                                 .arg(currentIdVente).arg(stagedVente.items.count()));
...
bool Contenir::commitVenteToDatabase(int idVente)
{
    ...
    for (const auto &item : stagedVente.items) {
        ...
        // Check if product exists in PRODUITS table
        QSqlQuery checkProdQuery;
        checkProdQuery.prepare("SELECT COUNT(*) FROM PRODUITS WHERE REFERENCE = :reference");
        ...
        checkProdQuery.next();
        if (checkProdQuery.value(0).toInt() == 0) {
        ...
        stockQuery.prepare("SELECT QUANTITE FROM PRODUITS WHERE REFERENCE = :reference");
        stockQuery.bindValue(":reference", refProduit);

        if (!stockQuery.exec()) {
            errorMessage = "Database error checking stock: " + stockQuery.lastError().text();
            qDebug() << errorMessage;
            allSuccess = false;
            break;
        }
        ...
        if (stockQuery.next()) {
            stock = stockQuery.value(0).toInt();
            qDebug() << "Stock for" << refProduit << ":" << stock;
        }
        ...
        // Get next ID using the fixed function
