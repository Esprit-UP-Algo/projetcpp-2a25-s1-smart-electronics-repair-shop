    ...
    networkManager(new QNetworkAccessManager(this))
{
    ...
    serial->setPortName("COM8");   // CHANGE COM port
...
void MainWindow::setupArduino()
{
    ...
    for (const QSerialPortInfo &portInfo : ports) {
        ...
        if (portInfo.portName() == "COM8") {
            com8Found = true;
            if (isBusy) {
                qDebug() << "   ⚠️ COM8 is BUSY (used by another program)!";
            }
        }
    ...
    if (!com8Found) {
        qDebug() << "   ❌ COM8 not found in available ports!";
        QMessageBox::critical(this, "COM8 Not Found",
                              "COM8 n'est pas disponible.\n\n"
...
{
    serialBuffer += QString(serial->readAll());

    // Process only when a full line is received
    if (!serialBuffer.contains("\n"))
        return;

    // Extract the full line
    QString message = serialBuffer.trimmed();
    serialBuffer.clear();  // reset buffer
...
void MainWindow::on_tableWidgetclient_cellClicked(int row)
{
    ...
    if (ui->tableWidgetclient->item(row, 8)) {
        ui->avis->setText(ui->tableWidgetclient->item(row, 8)->text());
    }
