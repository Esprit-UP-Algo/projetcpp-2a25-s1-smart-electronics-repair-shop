...
Client::Client(Ui::MainWindow *ui)
{
    ...
    this->setsexe(ui);
    this->avis = ui->avis->text();  // ADD THIS LINE - get from label
...
void Client::afficher(Ui::MainWindow *ui)
{
    ...
    if (query.exec())
    {
        ...
        while (query.next())
        {
            ...
            ui->tableWidgetclient->setItem(row, 8, new QTableWidgetItem(query.value("AVIS").toString())); // ADD THIS
            row++;
        }
    }
}
...
bool Client::modifier()
{
    ...
    query.prepare("UPDATE CLIENTS SET NOM=:nom, PRENOM=:prenom, ADRESSE=:adresse, NT=:nt, "
                  "BIRTH=:birth, SEXE=:sexe, AVIS=:avis "
                  "WHERE CIN=:cin");
    ...
    query.bindValue(":avis", avis);  // ADD THIS LINE

    bool test = query.exec();

    if (test)
        qDebug() << "✅ Modification réussie - Avis:" << avis;
    else
...
bool Client::ajouter()
{
    ...
    query.prepare(
        "INSERT INTO CLIENTS (CIN, NOM, PRENOM, ADRESSE, NT, BIRTH, SEXE, CREATIONDATE, AVIS) "
        "VALUES (:cin, :nom, :prenom, :adresse, :nt, "
    ...
    // ⚠ birth doit être "2025-12-08" ou un QDate converti
    query.bindValue(":birth", birth);

    query.bindValue(":sexe", sexe);

    // ⚠ creationdate doit être "2025-12-08"
    ...
    if (test) {
        qDebug() << "✅ Ajout réussi pour le client:" << nom << prenom << "Avis:" << avis;
    } else {
