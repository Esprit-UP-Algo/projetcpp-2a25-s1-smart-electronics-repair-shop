...
MainWindow::MainWindow(QWidget *parent) :
    ...
    Etmp(),
    networkManager(nullptr),  // ✅ Ajouté
    arduino(nullptr)  // ✅ IMPORTANT: Initialiser à nullptr
{
    ui->setupUi(this);
    ...
    keypadReferenceBuffer = "";
    selectedAppareilReference = "";
    isWaitingForStateChange = false;

    // ============================================
    // ARDUINO INITIALIZATION WITH FULL ERROR HANDLING
    // ============================================
    qDebug() << "--- Starting Arduino initialization ---";

    arduino = nullptr;  // Start with null
    initializeArduino();
    qDebug() << "\nARDUINO STATE AFTER INIT:";
    qDebug() << "  Pointer value:" << (void*)arduino;
    qDebug() << "  Is valid:" << (arduino != nullptr);

    if (arduino) {
        qDebug() << "  Serial exists:" << (arduino->getserial() != nullptr);
        if (arduino->getserial()) {
            qDebug() << "  Serial open:" << arduino->getserial()->isOpen();
        }
    }
    qDebug() << "--- Arduino initialization complete ---";
    qDebug() << "Final arduino pointer:" << (void*)arduino;

    if (arduino && arduino->getserial()) {
        qDebug() << "Serial state: Open =" << arduino->getserial()->isOpen();
    }
    if (arduino && arduino->getserial() && arduino->getserial()->isOpen()) {
        // Setup timer to read Arduino data continuously
        arduinoReadTimer = new QTimer(this);
        connect(arduinoReadTimer, &QTimer::timeout, this, &MainWindow::readArduinoData);
        arduinoReadTimer->start(100); // Check every 100ms

        qDebug() << "✅ Arduino read timer started";
    } else {
        arduinoReadTimer = nullptr;
        qDebug() << "⚠️ Arduino not ready, read timer not started";
    }
    // Don't call setupArduino() here - it conflicts with initializeArduino()
    // Instead, connect the signal to the existing serial port
    if (arduino && arduino->getserial() && arduino->getserial()->isOpen()) {
        connect(arduino->getserial(), &QSerialPort::readyRead,
                this, &MainWindow::onSerialDataReceived);
        qDebug() << "✅ Connected readyRead signal to existing Arduino serial port";
    }
    // ============================================
...
{
    // ✅ Check if Arduino is ready every time
    if (!isArduinoReady()) {
    ...
    qDebug() << "📨 Arduino sent (raw):" << data.toHex();
    qDebug() << "📨 Arduino sent (text):" << receivedText;
    qDebug() << "📨 Length:" << receivedText.length();

    // ✅ Check if it's an avis button press
    if (receivedText.startsWith("AVIS:")) {
        QString avisValue = receivedText.mid(5).trimmed(); // Remove "AVIS:" prefix

        qDebug() << "🔘 Button pressed! Avis value:" << avisValue;

        // ✅ Update the avis field in UI (check if it exists first!)
        if (ui->avis) {
            ui->avis->setText(avisValue);

            // ✅ Visual feedback with animation
            ui->avis->setStyleSheet(
                "QLineEdit { "
            ...
            // ✅ Show notification
            QMessageBox::information(this, "✅ Avis Reçu",
                                     QString("Avis sélectionné : %1").arg(avisValue));
        } else {
            qDebug() << "⚠️ WARNING: 'avis' QLineEdit not found in UI!";
            qDebug() << "   Please add a QLineEdit named 'avis' to your Qt Designer form";

            // ✅ Still show the value in a message box
            QMessageBox::information(this, "Avis Arduino",
                                     QString("Button pressed: %1\n\n(Add 'avis' QLineEdit to form to see value)").arg(avisValue));
        }
    }
    // ✅ Handle other messages (like LCD confirmations)
    else if (receivedText.contains("DISPLAYED:") ||
             receivedText.contains("MSG_RECEIVED") ||
             receivedText.contains("ARDUINO_READY")) {
        qDebug() << "ℹ️ Arduino status message:" << receivedText;
    }
    // ✅ Handle keypad input (single character keys)
    else if (receivedText.length() == 1) {
        QChar key = receivedText.at(0);
        qDebug() << "🔑 Processing single character key:" << key << "(Unicode:" << key.unicode() << ")";
        handleKeypadInput(key);
    }
    // Handle keypad input that might have newline/carriage return
    else if (receivedText.length() == 2 && (receivedText.endsWith("\n") || receivedText.endsWith("\r"))) {
        QChar key = receivedText.at(0);
        qDebug() << "🔑 Processing key with newline:" << key;
        handleKeypadInput(key);
    }
    else {
        qDebug() << "❓ Unknown message from Arduino:" << receivedText;
    }
}
...
// ============================================
void MainWindow::handleKeypadInput(QChar key)
{
    qDebug() << "🔑 Keypad key pressed:" << key;
    qDebug() << "   isWaitingForStateChange:" << isWaitingForStateChange;
    qDebug() << "   selectedAppareilReference:" << selectedAppareilReference;

    // Handle 'B' key: Update state of first row directly
    if (key == 'B' || key == 'b') {
        qDebug() << "🔑 'B' key pressed - updating first row state";
        
        QString refToUpdate = "";
        
        // If we already have a selected reference, use it
        if (!selectedAppareilReference.isEmpty()) {
            refToUpdate = selectedAppareilReference;
            qDebug() << "✅ Using already selected reference:" << refToUpdate;
        }
        // Otherwise, get the first row from the table
        else if (ui->tableWidgetfour->rowCount() > 0) {
            QTableWidgetItem *firstItem = ui->tableWidgetfour->item(0, 0);
            if (firstItem) {
                refToUpdate = firstItem->text();
                selectedAppareilReference = refToUpdate;
                isWaitingForStateChange = true;
                ui->tableWidgetfour->selectRow(0);
                ui->tableWidgetfour->setFocus();
                qDebug() << "✅ Auto-selected first row:" << refToUpdate;
            } else {
                qDebug() << "⚠️ First row item is null";
                QMessageBox::warning(this, "Erreur", 
                                     "Le tableau est vide ou la première ligne est invalide.\n"
                                     "Effectuez d'abord une recherche.");
                return;
            }
        } else {
            qDebug() << "⚠️ Table is empty";
            QMessageBox::information(this, "Tableau vide",
                                     "Le tableau est vide.\n"
                                     "Effectuez d'abord une recherche pour afficher des résultats.");
            return;
        }
        
        // Now update the state
        if (!refToUpdate.isEmpty()) {
            qDebug() << "🔄 Changing state for appareil:" << refToUpdate;
            
            // Set state to "en cours d'execution"
            appareils a;
            QString currentEtat = "";
            
            // Get current state from database
            QSqlQuery query;
            query.prepare("SELECT ETATAPPAREIL FROM APPAREILS WHERE REFERENCE = :ref");
            query.bindValue(":ref", refToUpdate);
            if (query.exec() && query.next()) {
                currentEtat = query.value(0).toString().trimmed();
                qDebug() << "📊 Current state:" << currentEtat;
            }
            
            // Set new state to "en cours d'execution"
            QString nouvelEtat = "en cours d'execution";
            
            qDebug() << "🔄 New state will be:" << nouvelEtat;
            
            // Change the state
            if (a.changerEtat(refToUpdate, nouvelEtat)) {
                qDebug() << "✅ State changed successfully in database";
                
                // Refresh the appareils table to show updated state
                a.afficher(ui);
                
                // Re-select the row if it still exists
                for (int i = 0; i < ui->tableWidgetfour->rowCount(); ++i) {
                    if (ui->tableWidgetfour->item(i, 0) && 
                        ui->tableWidgetfour->item(i, 0)->text() == refToUpdate) {
                        ui->tableWidgetfour->selectRow(i);
                        break;
                    }
                }
                
                QMessageBox::information(this, "✅ État modifié",
                                         QString("État de l'appareil %1 changé:\n%2 → %3")
                                         .arg(refToUpdate)
                                         .arg(currentEtat.isEmpty() ? "Vide" : currentEtat)
                                         .arg(nouvelEtat));
                
                // Send confirmation to LCD
                sendToLCD(QString("Etat: %1").arg(nouvelEtat));
                
                // Keep the selection active for another state change if needed
                isWaitingForStateChange = true;
            } else {
                qDebug() << "❌ Failed to change state";
                QMessageBox::warning(this, "Erreur", 
                                     QString("Impossible de changer l'état de l'appareil %1.\nVérifiez que la référence existe.")
                                     .arg(refToUpdate));
            }
        }
        return;
    ...
    // Accept digits (0-9) and letters (A-Z) for reference
    if ((key >= '0' && key <= '9') || (key >= 'A' && key <= 'Z') || (key >= 'a' && key <= 'z')) {
        // Reset state change mode when starting new input
        isWaitingForStateChange = false;
        selectedAppareilReference = "";
        
        // Add to buffer (max 8 characters for reference)
        if (keypadReferenceBuffer.length() < 8) {
            keypadReferenceBuffer += key.toUpper();
            qDebug() << "📝 Reference buffer:" << keypadReferenceBuffer;
            qDebug() << "📝 Buffer length:" << keypadReferenceBuffer.length();
            
            // Update UI field - ALWAYS update to ensure synchronization
            bool fieldUpdated = false;
            
            // Try lineEdit_34 first (appareils search field)
            if (ui->lineEdit_34) {
                // Clear and set to ensure proper update
                ui->lineEdit_34->clear();
                ui->lineEdit_34->setText(keypadReferenceBuffer);
                ui->lineEdit_34->setFocus(); // Ensure field is focused
                fieldUpdated = true;
                qDebug() << "✅ Updated lineEdit_34 with:" << keypadReferenceBuffer;
            }
            
            // If not found, try to update any visible search field on fournisseur page
            if (!fieldUpdated) {
                qDebug() << "⚠️ lineEdit_34 not found, checking for other search fields";
                // The field might be named differently, but we'll log it for debugging
            }
            
            // Force UI update - ALWAYS do this to ensure display is updated
            if (fieldUpdated) {
                ui->lineEdit_34->repaint();
                ui->lineEdit_34->update(); // Force update
                qApp->processEvents(); // Process events to update UI immediately
                qDebug() << "✅ UI field forced update completed";
            }
            
            // Send to LCD for visual feedback
            sendToLCD(QString("Ref: %1").arg(keypadReferenceBuffer));
            
            // If buffer is full (8 characters), automatically search
            if (keypadReferenceBuffer.length() == 8) {
                qDebug() << "🔍 Auto-searching for reference:" << keypadReferenceBuffer;
                QTimer::singleShot(100, this, [this]() {
                    QString refToSearch = keypadReferenceBuffer; // Save current buffer
                    searchAppareilByReference(refToSearch);
                    // Don't clear buffer immediately - keep it visible
                    // keypadReferenceBuffer = "";
                });
            }
        } else {
            qDebug() << "⚠️ Reference buffer full, ignoring key";
            QMessageBox::information(this, "Buffer plein", 
                                     "La référence est complète (8 caractères).\nAppuyez sur '#' pour rechercher ou '*' pour effacer.");
        }
    }
    // '*' key: Clear buffer
    else if (key == '*') {
        ...
        qDebug() << "🗑️ Buffer cleared";
        sendToLCD("Buffer cleared");
    }
    // '#' key: Confirm and search
    else if (key == '#') {
        if (!keypadReferenceBuffer.isEmpty()) {
            qDebug() << "🔍 Searching for reference:" << keypadReferenceBuffer;
            searchAppareilByReference(keypadReferenceBuffer);
            // Don't clear buffer immediately - let user see what was searched
            // keypadReferenceBuffer = "";
        } else {
            QMessageBox::information(this, "Recherche", "Buffer vide. Entrez une référence d'abord.");
        }
    }
    // 'C' key: Cancel
    else if (key == 'C') {
    ...
    // 'A' key: Select first row in table (after search)
    else if (key == 'A' || key == 'a') {
        qDebug() << "🔑 'A' key pressed - selecting first row";
        
        // Check if table has any rows
        if (ui->tableWidgetfour->rowCount() > 0) {
            // Get the reference from the first row
            QTableWidgetItem *firstItem = ui->tableWidgetfour->item(0, 0);
            if (firstItem) {
                selectedAppareilReference = firstItem->text();
                isWaitingForStateChange = true;
                ...
                QString currentEtat = "";
                if (ui->tableWidgetfour->item(0, 4)) {
                    currentEtat = ui->tableWidgetfour->item(0, 4)->text();
                }
                
                qDebug() << "✅ First row selected - Reference:" << selectedAppareilReference;
                qDebug() << "   État actuel:" << currentEtat;
                qDebug() << "   isWaitingForStateChange set to: true";
                
                // Send to LCD
                sendToLCD(QString("Sel: %1").arg(selectedAppareilReference));
                
                // Show brief notification (non-blocking)
                QMessageBox::information(this, "✅ Appareil sélectionné",
                                         QString("Premier appareil sélectionné:\n\n"
                                                 "Référence: %1\n"
                                                 "État actuel: %2\n\n"
                                                 "Appuyez sur 'B' pour mettre à jour l'état")
                                         .arg(selectedAppareilReference)
                                         .arg(currentEtat.isEmpty() ? "Non défini" : currentEtat));
            } else {
                qDebug() << "⚠️ First row item is null";
                QMessageBox::warning(this, "Erreur", "Impossible de lire la référence de la première ligne.");
            }
        } else {
            qDebug() << "⚠️ Table is empty";
            QMessageBox::information(this, "Tableau vide",
        ...
        return; // Important: return after handling 'A'
    }
}

// ============================================
// SEARCH APPAREIL BY REFERENCE
// ============================================
void MainWindow::searchAppareilByReference(const QString &reference)
{
    if (reference.isEmpty()) {
        qDebug() << "⚠️ Empty reference for search";
        return;
    }

    qDebug() << "🔍 Searching for reference:" << reference;
    
    // Make sure we're on the fournisseur page
    ui->stackedWidget->setCurrentWidget(ui->fournisseur);
    ui->stackedWidgetfour->setCurrentIndex(0);

    appareils a;
    bool found = a.rechercherParReference(ui->tableWidgetfour, reference);

    if (found) {
        int rowCount = ui->tableWidgetfour->rowCount();
        qDebug() << "✅ Found" << rowCount << "result(s)";
        
        // If exactly one result, select it automatically
        if (rowCount == 1) {
            selectedAppareilReference = reference;
            isWaitingForStateChange = true;
            ...
            // Get the actual reference from the table (might be different if partial match)
            if (ui->tableWidgetfour->item(0, 0)) {
                selectedAppareilReference = ui->tableWidgetfour->item(0, 0)->text();
            }
            
            qDebug() << "✅ Auto-selected reference:" << selectedAppareilReference;
            
            QMessageBox::information(this, "✅ Appareil trouvé",
                                     QString("Référence trouvée: %1\n\nAppuyez sur 'B' pour changer l'état.")
                                     .arg(selectedAppareilReference));
            
            sendToLCD(QString("Found: %1").arg(selectedAppareilReference));
        } else {
            // Multiple results - user needs to select one
            qDebug() << "📋 Multiple results, waiting for user selection";
            QMessageBox::information(this, "Recherche",
                                     QString("%1 résultat(s) trouvé(s) pour: %1\n\n"
                                             "Sélectionnez un appareil dans le tableau puis appuyez sur 'B' pour changer l'état.")
                                     .arg(rowCount)
                                     .arg(reference));
            sendToLCD(QString("Found: %1").arg(rowCount));
        }
    } else {
        qDebug() << "❌ No results found for:" << reference;
        QMessageBox::warning(this, "Non trouvé", 
                             QString("Aucun appareil trouvé avec la référence: %1\n\n"
                                     "Appuyez sur 'A' pour voir tous les appareils.")
                                     .arg(reference));
        sendToLCD("Not found");
    }
}
void MainWindow::initializeArduino()
{
    ...
    // S'assurer que le pointeur est null au départ
    arduino = nullptr;

    try {
        qDebug() << "Step 1: Allocating Arduino object...";

        // Allouer l'objet Arduino
        arduino = new Arduino();

        // Vérification immédiate
        if (!arduino) {
            qDebug() << "ERROR: new Arduino() returned nullptr!";
            QMessageBox::critical(this, "Erreur Critique",
                                  "Impossible d'allouer la mémoire pour Arduino!\n"
                                  "Le programme continuera sans LCD.");
            return;
        }

        qDebug() << "Step 2: Arduino object allocated at:" << (void*)arduino;

        // Vérifier que l'objet est valide en accédant à une méthode simple
        QString portName = arduino->getarduino_port_name();
        qDebug() << "Step 3: Port name retrieved:" << portName;

        // Tenter la connexion
        qDebug() << "Step 4: Attempting connection...";
        int result = arduino->connect_arduino();

        qDebug() << "Step 5: Connection result:" << result;
        ...
        if (result == 0) {
            ...
            if (serial && serial->isOpen()) {
                qDebug() << "✅ Serial port verified open";
                qDebug() << "   Port name:" << serial->portName();
                qDebug() << "   Baud rate:" << serial->baudRate();

                // Envoyer un message d'initialisation
                QByteArray initMsg = "READY\n";
                arduino->write_to_arduino(initMsg);
                serial->flush();

                QMessageBox::information(this, "Arduino OK",
                                         "✅ Arduino connecté avec succès!\n\n"
                                         "Port: " + arduino->getarduino_port_name());

            } else {
                qDebug() << "⚠️ Warning: Serial port not properly opened";
            }
            ...
            QMessageBox::warning(this, "Arduino",
                                 "⚠️ Impossible de se connecter à l'Arduino.\n\n"
                                 "Vérifiez:\n"
                                 "• Arduino branché sur le port USB\n"
                                 "• Bon port COM dans arduino.cpp\n"
        ...
        qDebug() << "❌ EXCEPTION: Memory allocation failed:" << e.what();
        arduino = nullptr;
        QMessageBox::critical(this, "Erreur Mémoire",
                              "Échec d'allocation mémoire pour Arduino!");

    } catch (const std::exception &e) {
        qDebug() << "❌ EXCEPTION:" << e.what();
        arduino = nullptr;
        QMessageBox::critical(this, "Exception",
                              QString("Exception durant l'initialisation:\n%1").arg(e.what()));

    } catch (...) {
        qDebug() << "❌ UNKNOWN EXCEPTION during Arduino init";
        arduino = nullptr;
        QMessageBox::critical(this, "Exception",
                              "Exception inconnue durant l'initialisation Arduino!");
    }

    qDebug() << "--- ARDUINO INITIALIZATION END ---";
    qDebug() << "Final pointer value:" << (void*)arduino << "\n";
}

// ============================================
// VÉRIFICATION SI ARDUINO EST PRÊT
// ============================================

bool MainWindow::isArduinoReady()
{
    if (!arduino) {
        qDebug() << "isArduinoReady: arduino is nullptr";
        return false;
    }

    QSerialPort *serial = arduino->getserial();
    if (!serial) {
        qDebug() << "isArduinoReady: serial is nullptr";
        return false;
    }

    if (!serial->isOpen()) {
        qDebug() << "isArduinoReady: serial port not open";
        return false;
    }
...
// ============================================
// ENVOI VERS LCD (FONCTION UTILITAIRE)
// ============================================

void MainWindow::sendToLCD(const QString &message)
{
    qDebug() << "\n--- sendToLCD called ---";
    qDebug() << "Message:" << message;

    if (!isArduinoReady()) {
        qDebug() << "Arduino not ready, skipping LCD send";
        return;
    }

    try {
        QByteArray data = message.toUtf8() + "\n";
        qDebug() << "Sending" << data.size() << "bytes to Arduino";
        ...
        if (arduino->getserial()->waitForBytesWritten(1000)) {
            ...
            qDebug() << "⚠️ Timeout waiting for bytes to be written";
        }

    } catch (...) {
        qDebug() << "❌ Exception during LCD send";
    }

    qDebug() << "--- sendToLCD end ---\n";
}
...
{
    // Check if column 5 was clicked (columns are 0-indexed, so column 5 = 6th column)
    if (column == 5)
    {
        // Take the reference from column 0 (first column)
    ...
    {
        // When clicking on any cell, set the selected reference for keypad state change
        if (row >= 0 && ui->tableWidgetfour->item(row, 0)) {
            selectedAppareilReference = ui->tableWidgetfour->item(row, 0)->text();
            isWaitingForStateChange = true;
            
            // Get current state for display
            QString currentEtat = "";
            if (ui->tableWidgetfour->item(row, 4)) {
                currentEtat = ui->tableWidgetfour->item(row, 4)->text();
            }
            
            qDebug() << "📱 Appareil sélectionné:" << selectedAppareilReference;
            qDebug() << "   État actuel:" << currentEtat;
            qDebug() << "   isWaitingForStateChange set to: true";
            qDebug() << "   Appuyez sur 'B' pour changer l'état";
            
            // Visual feedback - highlight the row
            ui->tableWidgetfour->selectRow(row);
            ui->tableWidgetfour->setFocus();
            ...
            // Show brief notification (non-blocking)
            QMessageBox::information(this, "Appareil sélectionné",
                                     QString("Référence: %1\nÉtat actuel: %2\n\n✅ Prêt! Appuyez sur 'B' pour changer l'état")
                                     .arg(selectedAppareilReference)
                                     .arg(currentEtat.isEmpty() ? "Non défini" : currentEtat));
        }
    }
...
// ============================================
// SECOND CONSTRUCTOR - VERSION CORRIGÉE
// ============================================
...
MainWindow::MainWindow(const UserInfo &userInfo, QWidget *parent) :
    ...
    networkManager(nullptr),  // ✅ Ajouté
    arduino(nullptr)          // ✅✅✅ CRITIQUE - MANQUAIT!
{
    qDebug() << "\n========================================";
    qDebug() << "MAINWINDOW CONSTRUCTOR (WITH USER) START";
    qDebug() << "========================================";
    ...
    // ============================================
    // INITIALIZE ARDUINO - ✅✅✅ MANQUAIT COMPLÈTEMENT!
    // ============================================
    qDebug() << "--- Starting Arduino initialization ---";
    arduino = nullptr;  // Start with null
    initializeArduino();
    ...
    qDebug() << "========================================";
    qDebug() << "MAINWINDOW CONSTRUCTOR (WITH USER) END";
    qDebug() << "Final arduino pointer:" << (void*)arduino;
    qDebug() << "========================================\n";
}
void MainWindow::setupArduino()
{
    qDebug() << "";
    qDebug() << "╔════════════════════════════════════╗";
    qDebug() << "║   ARDUINO SETUP (LEGACY)          ║";
    qDebug() << "╚════════════════════════════════════╝";

    // Check if Arduino is already connected via initializeArduino()
    if (arduino && arduino->getserial() && arduino->getserial()->isOpen()) {
        qDebug() << "✅ Arduino already connected via initializeArduino()";
        qDebug() << "   Port:" << arduino->getserial()->portName();
        qDebug() << "   Using existing serial port connection";
        
        // Use the existing serial port
        serial = arduino->getserial();
        
        // Connect the signal if not already connected
        if (!serial->signalsBlocked()) {
        ...
        if (ui->avis) {
            qDebug() << "✅ Label 'avis' found";
            ui->avis->setText("Arduino Prêt");
            ui->avis->setStyleSheet("color: green; font-weight: bold; font-size: 14px;");
        }
        
        QMessageBox::information(this, "✅ Arduino Connecté",
                                 QString("Arduino connecté sur %1\n\n"
                                         "Appuyez sur les boutons du keypad pour tester!")
                                     .arg(serial->portName()));
        return;
    }

    // Legacy code - only used if Arduino wasn't initialized via initializeArduino()
    ...
    if (ports.isEmpty()) {
        ...
        QMessageBox::warning(this, "Arduino Warning",
                             "Aucun port COM détecté!");
        return;
    }

    QString arduinoPort = "";
    bool com8Found = false;
    ...
    for (const QSerialPortInfo &portInfo : ports) {
        ...
        QSerialPort test;
        test.setPort(portInfo);
        bool isBusy = !test.open(QIODevice::ReadWrite);
        if (!isBusy) test.close();

        qDebug() << "      Busy:" << isBusy;

        // Check for both COM8 and COM9
        if (portInfo.portName() == "COM8" || portInfo.portName() == "COM9") {
            if (portInfo.portName() == "COM9") {
                com8Found = true; // Use same flag for COM9
                qDebug() << "   ✅ COM9 found!";
            } else {
                com8Found = true;
            }
            if (isBusy) {
                qDebug() << "   ⚠️ " << portInfo.portName() << " is BUSY (used by another program)!";
            }
        }
        ...
        if (portInfo.description().contains("Arduino", Qt::CaseInsensitive) ||
            ...
            portInfo.vendorIdentifier() == 0x2341 ||
            portInfo.vendorIdentifier() == 0x1a86 ||
            portInfo.vendorIdentifier() == 9025) {

            arduinoPort = portInfo.portName();
            qDebug() << "   ✅ ARDUINO DETECTED:" << arduinoPort;
        }
    }

    if (!com8Found) {
        qDebug() << "   ❌ COM8/COM9 not found in available ports!";
        QMessageBox::critical(this, "Port COM Not Found",
                              "COM8 ou COM9 n'est pas disponible.\n\n"
                              "L'Arduino est peut-être débranché.");
        return;
    }

    // Use COM9 if available, otherwise COM8
    if (arduinoPort.isEmpty()) {
        // Check if COM9 exists
        bool com9Exists = false;
        for (const QSerialPortInfo &portInfo : ports) {
            if (portInfo.portName() == "COM9") {
                com9Exists = true;
                break;
        ...
        arduinoPort = com9Exists ? "COM9" : "COM8";
        qDebug() << "   Using port:" << arduinoPort;
    }
    ...
    qDebug() << "🔌 Attempting to open" << arduinoPort << "...";

    // ✅ ADD EXCLUSIVE OPEN MODE
    if (serial->open(QSerialPort::ReadWrite)) {
        qDebug() << "✅ SUCCESS! Serial port opened";
        qDebug() << "   Port:" << serial->portName();
        qDebug() << "   Baud Rate:" << serial->baudRate();

        // Clear any existing data
        serial->clear();
       // QThread::msleep(100); // Small delay for Arduino to reset
        ...
        QMessageBox::information(this, "✅ Arduino Connecté",
                                 QString("Arduino connecté sur %1\n\n"
                                         "Appuyez sur les boutons (pins 8, 9, 10) pour tester!")
                                     .arg(arduinoPort));
        ...
        case QSerialPort::DeviceNotFoundError:
            errorDetails = "Appareil non trouvé";
            break;
        case QSerialPort::PermissionError:
            errorDetails = "Accès refusé (utilisé par un autre programme)";
            break;
        case QSerialPort::OpenError:
            errorDetails = "Impossible d'ouvrir le port";
            break;
        case QSerialPort::NotOpenError:
            errorDetails = "Port non ouvert";
            break;
        default:
            errorDetails = serial->errorString();
        }
        ...
        QMessageBox::critical(this, "❌ Erreur d'ouverture",
                              QString("Impossible d'ouvrir %1\n\n"
                                      ...
                                      "2. FERMEZ le Serial Monitor\n"
                                      "3. Débranchez l'Arduino, attendez 5 sec, rebranchez\n"
                                      "4. Redémarrez cette application\n\n"
                                      "Si le problème persiste:\n"
                                      "• Ouvrez le Gestionnaire de périphériques\n"
                                      "• Ports COM → Arduino → Clic droit → Désactiver\n"
                                      "• Puis Activer à nouveau")
                                  .arg(arduinoPort)
                                  .arg(errorDetails));
    }
...
{
    // Use the Arduino's serial port if available, otherwise use the legacy serial
    QSerialPort *port = nullptr;
    if (arduino && arduino->getserial() && arduino->getserial()->isOpen()) {
        port = arduino->getserial();
    } else if (serial && serial->isOpen()) {
        port = serial;
    } else {
        qDebug() << "⚠️ No serial port available in onSerialDataReceived";
        return;
    }
    
    serialBuffer += QString(port->readAll());

    // Process line by line
    while (serialBuffer.contains("\n")) {
        int newlinePos = serialBuffer.indexOf("\n");
        QString message = serialBuffer.left(newlinePos).trimmed();
        serialBuffer = serialBuffer.mid(newlinePos + 1);  // Remove processed line

        if (message.isEmpty()) continue;

        qDebug() << "Arduino message:" << message;

        // Handle AVIS messages
        if (message.startsWith("AVIS:")) {
            QString avisText = message.mid(5).trimmed();
            if (ui->avis) {
                ui->avis->setText(avisText);

                // Color the text
                if (avisText == "Passable")
                    ui->avis->setStyleSheet("color: orange; font-weight: bold; font-size: 16px;");
                else if (avisText == "Mediocre")
                    ui->avis->setStyleSheet("color: red; font-weight: bold; font-size: 16px;");
                else if (avisText == "Satisfait")
                    ui->avis->setStyleSheet("color: green; font-weight: bold; font-size: 16px;");
            }
        }
        // Handle single character keypad input
        else if (message.length() == 1) {
            QChar key = message.at(0);
            qDebug() << "🔑 Processing key from onSerialDataReceived:" << key;
            handleKeypadInput(key);
        }
        // Handle keypad input that might have newline/carriage return
        else if (message.length() == 2 && (message.endsWith("\n") || message.endsWith("\r"))) {
            QChar key = message.at(0);
            qDebug() << "🔑 Processing key with newline from onSerialDataReceived:" << key;
            handleKeypadInput(key);
        }
        // Handle other status messages
        else if (message.contains("DISPLAYED:") ||
                 message.contains("MSG_RECEIVED") ||
                 message.contains("ARDUINO_READY")) {
            qDebug() << "ℹ️ Arduino status message:" << message;
        }
        else {
            if (ui->avis) {
                ui->avis->setText(message);
                ui->avis->setStyleSheet("color: blue; font-weight: bold; font-size: 14px;");
            }
        }
    }
}
...
MainWindow::~MainWindow()
{
    ...
    if (arduino) {
        qDebug() << "Deleting Arduino object at:" << (void*)arduino;
        try {
            delete arduino;
            arduino = nullptr;
            qDebug() << "✅ Arduino deleted successfully";
        } catch (...) {
            qDebug() << "❌ Exception while deleting Arduino";
        }
    } else {
        qDebug() << "Arduino was nullptr, nothing to delete";
    }
...
MainWindow::~MainWindow()
{
    ...
    if (arduinoReadTimer) {
        arduinoReadTimer->stop();
        delete arduinoReadTimer;
        arduinoReadTimer = nullptr;
        qDebug() << "Arduino read timer stopped and deleted";
    }
...
void MainWindow::on_pushButton_ValiderClient_clicked()
{
    ...
    QString avis = c.getavis().trimmed();
    if (avis.isEmpty()) {
        QMessageBox::warning(this, "Erreur",
                             "Le champ 'Avis' est vide !\n\n"
                             "Veuillez soit:\n"
                             "• Appuyer sur un bouton Arduino (Excellent/Passable/Mauvais)\n"
                             "• Ou saisir manuellement un avis");
        test = false;
    }

    // ✅ Optional: Validate avis is one of the expected values
    QStringList validAvis = {"Excellent", "Passable", "Mauvais"};
    if (!avis.isEmpty() && !validAvis.contains(avis, Qt::CaseInsensitive)) {
        QMessageBox::StandardButton reply = QMessageBox::question(
            this,
            "Avis non standard",
            QString("L'avis '%1' n'est pas standard.\n\n"
                    "Avis standards: Excellent, Passable, Mauvais\n\n"
                    "Voulez-vous continuer quand même ?").arg(avis),
    ...
    if (test)
    {
        ...
        else
        {
            ...
            if (ajoutReussi) {
                ...
                // ============================================
                // SEND TO LCD DISPLAY - SAFE VERSION
                // ============================================
                QString lcdMessage = c.getnom() + " " + c.getprenom() + " - " + c.getavis();

                qDebug() << "=== ATTEMPTING TO SEND TO LCD ===";
                qDebug() << "Message:" << lcdMessage;

                // Use the safe sendToLCD function
                sendToLCD(lcdMessage);

                qDebug() << "=== LCD SEND COMPLETED ===";
                ...
                QMessageBox::critical(this, "Erreur", "Client n'est pas ajouté");
            }
        }
    }
...
{
    qDebug() << "\n========================================";
    qDebug() << "ARDUINO TEST BUTTON CLICKED";
    qDebug() << "========================================";

    qDebug() << "\nCHECK 1: Arduino pointer";
    qDebug() << "  Address:" << (void*)arduino;
    qDebug() << "  Is null:" << (arduino == nullptr);

    if (!arduino) {
        qDebug() << "ERROR: Arduino pointer is NULL!";
        QMessageBox::critical(this, "Erreur",
                              "❌ L'objet Arduino n'existe pas!\n\n"
                              "L'Arduino n'a pas été initialisé correctement.\n\n"
    ...
    qDebug() << "  Expected: COM9";
    
    if (portName != "COM9") {
        QMessageBox::warning(this, "Port incorrect",
                             QString("Le port Arduino est configuré sur: %1\n\n"
                                     "Attendu: COM9\n\n"
                                     "Vérifiez arduino.cpp pour confirmer le port.")
                             .arg(portName));
    }

    qDebug() << "\nCHECK 2: Serial port object";
    QSerialPort *serial = nullptr;

    try {
        serial = arduino->getserial();
        qDebug() << "  Serial address:" << (void*)serial;
        qDebug() << "  Is null:" << (serial == nullptr);
    } catch (...) {
        qDebug() << "❌ EXCEPTION while getting serial port!";
        QMessageBox::critical(this, "Exception",
                              "Exception lors de l'accès au port série!");
        return;
    }

    if (!serial) {
        qDebug() << "ERROR: Serial port is NULL!";
        QMessageBox::critical(this, "Erreur",
                              "❌ Le port série n'existe pas!");
        return;
    }

    qDebug() << "\nCHECK 3: Serial port state";
    ...
    if (!isOpen) {
        QMessageBox::StandardButton reply = QMessageBox::question(this,
                                                                  ...
                                                                  "Voulez-vous essayer de le reconnecter?",
                                                                  QMessageBox::Yes | QMessageBox::No);
        ...
        if (reply == QMessageBox::Yes) {
            ...
            if (result == 0) {
                ...
                QMessageBox::critical(this, "Échec",
                                      "❌ Impossible de reconnecter l'Arduino!\n\n"
                                      "Vérifiez les connexions et redémarrez.");
                return;
            }
        } else {
            return;
    ...
    try {
        QString testMsg = "TEST OK!";
        QByteArray data = testMsg.toUtf8() + "\n";

        qDebug() << "  Message:" << testMsg;
        qDebug() << "  Bytes:" << data.toHex();

        arduino->write_to_arduino(data);

        bool written = serial->waitForBytesWritten(2000);

        if (written) {
            qDebug() << "✅ Test message sent successfully!";
            QMessageBox::information(this, "Test OK",
                                     "✅ Message envoyé avec succès!\n\n"
                                     "Vérifiez l'afficheur LCD:\n" + testMsg);
        } else {
            qDebug() << "⚠️ Timeout or error";
            qDebug() << "  Error:" << serial->errorString();
            QMessageBox::warning(this, "Attention",
                                 "⚠️ Timeout lors de l'envoi.\n\n"
