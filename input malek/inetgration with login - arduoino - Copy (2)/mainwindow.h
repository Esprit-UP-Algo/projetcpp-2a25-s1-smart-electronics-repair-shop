...
private slots:
    ...
    void handleKeypadInput(QChar key);
    void searchAppareilByReference(const QString &reference);
...
private:
    ...
    QTimer *arduinoReadTimer;
    QSerialPort *serial;

    void setupArduino();QString serialBuffer;
    
    // Keypad integration for appareils
    QString keypadReferenceBuffer;  // Buffer to accumulate reference input
    QString selectedAppareilReference;  // Currently selected appareil reference
    bool isWaitingForStateChange;  // Flag to indicate waiting for 'B' key
};
#endif // MAINWINDOW_H
